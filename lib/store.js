...
async function getGroupMetadata(conn, id) {
    if (groupMetadataCache[id]) {
        return groupMetadataCache[id]
    }

    const metadata = await Promise.race([
        conn.groupMetadata(id).catch(_ => null),
        new Promise(resolve => setTimeout(() => resolve(null), 10000))
    ])

    if (metadata) {
        groupMetadataCache[id] = metadata
    }
...
function bind(conn) {
    ...
    conn.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        ...
        const groupMetadata = await getGroupMetadata(conn, id)
        if (groupMetadata) {
    ...
    conn.ev.on('chats.upsert', function chatsUpsertPushToDb(chatsUpsert) {
        try {
            ...
            if (isGroup) {
                try {
                    conn.insertAllGroup().catch(_ => null)
                } catch (e) {}
            }
        } catch (e) {}
    })
