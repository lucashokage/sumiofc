...
export default async function (m, conn = { user: {} }) {
  try {
    if (!conn || !conn.user || !conn.user.jid || !global.conn || !global.conn.user || conn.user.jid !== global.conn.user.jid) return;

    let _name = await conn.getName(m.sender).catch(() => '')
    let sender = m.sender ? PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '') : ''
    let chat = await conn.getName(m.chat).catch(() => '')
    
    let img
    try {
      if (global.opts['img'] && m.download)
    ...
    let me = conn.user?.jid ? PhoneNumber('+' + conn.user.jid.replace('@s.whatsapp.net', '')).getNumber('international') : ''
...
`.trim(), 
    ...
    if (m.messageStubParameters) {
      console.log(m.messageStubParameters.map(jid => {
        ...
        let name = conn.getName(jid).catch(() => '')
    ...
    console.error('Error en print.js:', e)
  }
}
