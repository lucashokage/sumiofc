...
/**
 * Simplified message object
 * @param {import('@whiskeysockets/baileys').proto.IWebMessageInfo} m
 * @param {Socket} conn
 * @returns {import('@whiskeysockets/baileys').proto.IWebMessageInfo}
 */
export function smsg(conn, m, store) {
  if (!m) return m
  const M = proto.WebMessageInfo

  // Create a new object instead of modifying the original message
  const msg = {}
  ...
  if (m.key) {
    ...
    msg.sender = conn.decodeJid(m.fromMe && conn.user ? conn.user.jid : m.key.participant || m.chat || "")
  }

  if (m.message) {
    msg.mtype = Object.keys(m.message)[0]
    msg.msg = m.message[msg.mtype]
    if (msg.mtype == "ephemeralMessage") {
      smsg(conn, msg.msg)
      msg.mtype = msg.msg.mtype
      msg.msg = msg.msg.msg
    }
    msg.text =
      msg.msg?.text || msg.msg?.caption || msg.msg?.jpegThumbnail || msg.msg?.selectedDisplayText || msg.msg?.url || ""
    if (typeof msg.text !== "string")
      msg.text =
        (msg.mtype === "viewOnceMessage" ? msg.msg?.viewOnceMessage?.message : m.message[msg.mtype])?.text || ""
    msg.name = m.pushName || "Sin Nombre"
    if (msg.msg?.url) msg.text += "\n" + msg.msg.url
  }

  // Copy all properties from the original message
  Object.assign(msg, m)

  msg.reply = async (text, chatId, options) => {
    return await conn.sendMessage(chatId ? chatId : msg.chat, { text: text }, { quoted: m, ...options })
  }
...
// Canal de newsletter para reenvío
...
export function makeWASocket(connectionOptions, options = {}) {
  ...
  // Crear una instancia de EventEmitter para manejar eventos
  const ev = conn.ev
    ...
    sendFile: {
      ...
      async value(jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) {
        try {
          ...
          // Configurar como mensaje reenviado de canal
...
END:VCARD
    ...
    reply: {
      ...
      value(jid, text = "", quoted, options = {}) {
        // Configurar como mensaje reenviado de canal si se solicita
        if (options.asChannel || options.asNewsletter) {
          const viewOnceMessage = {
            message: {
        ...
        try {
          ...
          if (buffer && /^https?:\/\//i.test(buffer)) {
            try {
              ...
              } else if (contentType && /^video\//i.test(contentType)) {
            ...
            try {
              ...
              if (type && type.mime && /^image\//i.test(type.mime)) {
                img = await prepareWAMessageMedia({ image: { url: buffer } }, { upload: conn.waUploadToServer })
              } else if (type && type.mime && /^video\//i.test(type.mime)) {
                video = await prepareWAMessageMedia({ video: { url: buffer } }, { upload: conn.waUploadToServer })
              }
            } catch (error) {
    ...
    sendHydrated: {
      ...
      async value(jid, text = "", footer = "", buffer, url, urlText, call, callText, buttons, quoted, options) {
        try {
          ...
          if (call_ || callText_) {
            if (!Array.isArray(call_)) call_ = [call_]
            if (!Array.isArray(callText_)) callText_ = [callText_]
            templateButtons.push(
              ...(call_
                .map((v, i) => [v, callText_[i]])
                .map(([call, callText], i) => ({
                  index: templateButtons.length + i + 1,
                  callButton: {
    ...
    processMessageStubType: {
      ...
      async value(m) {
        try {
          ...
          // Usar el evento de la instancia de conn
      ...
      async value() {
        try {
          ...
          for (let i = 0; i < groupEntries.length; i += batchSize) {
            ...
            for (const [group, groupData] of batch) {
              conn.chats[group] = {
                ...(conn.chats[group] || {}),
                id: group,
    ...
    serializeM: {
       ...
       */
      value(m) {
        return safeProcessMessage(conn, m)
      },
    },
 ...
 */
export function processMessage(conn, m, hasParent) {
  if (!m) return m
  /**
   * @type {import('@whiskeysockets/baileys').proto.WebMessageInfo}
   */
...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
      ...
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === "string" ? { text: q } : q)), {
          ...
          copy: {
            ...
            value() {
              const M = proto.WebMessageInfo
              return processMessage(this.conn, M.fromObject(M.toObject(this.vM)))
            },
            enumerable: true,
          },
          forward: {
    ...
    copy: {
      value() {
        const M = proto.WebMessageInfo
        return processMessage(this.conn, M.fromObject(M.toObject(this)))
      },
      enumerable: true,
    },
    forward: {
    ...
    getQuotedObj: {
      value() {
        ...
        return processMessage(this.conn, q)
      },
      enumerable: true,
    },
    getQuotedMessage: {
...
// Add this function at the end of the file, before the last export
export function safeProcessMessage(conn, m) {
  try {
    return processMessage(conn, m)
  } catch (error) {
    console.error("Error in processMessage:", error)
    // Return a minimal processed message to prevent crashes
    const safeMsg = { ...m }
    safeMsg.conn = conn
    return safeMsg
  }
