...
export function smsg(conn, m, store) {
  ...
  if (m.key) {
    ...
    m.sender = conn.decodeJid(m.fromMe && conn.user ? conn.user.jid : m.key.participant || m.chat || "")
  }
  if (m.message) {
    ...
    m.text = m.msg?.text || m.msg?.caption || m.msg?.jpegThumbnail || m.msg?.selectedDisplayText || m.msg?.url || ""
    if (typeof m.text !== "string")
      m.text = (m.mtype === "viewOnceMessage" ? m.msg?.viewOnceMessage?.message : m.message[m.mtype])?.text || ""
    m.name = m.pushName || "Sin Nombre"
    if (m.msg?.url) m.text += "\n" + m.msg.url
  }
  m.reply = async (text, chatId, options) => {
    return await conn.sendMessage(chatId ? chatId : m.chat, { text: text }, { quoted: m, ...options })
  }
  return m
}
