...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const groupMetadata =
      (m.isGroup
        ? (global.conn.chats[m.chat] || {}).metadata || (await this.groupMetadata(m.chat).catch(() => null))
        : {}) || {}
...
export async function participantsUpdate({ id, participants, action }) {
  ...
  if (action === "promote" || action === "demote") {
    ...
    if (action === "promote") {
      ...
      text = chat.sDemote || this.sdemote || global.conn.sdemote || "=͟͟͞❀ @user 𝙮𝙖 𝙣𝙤 𝙚𝙨 𝙖𝙙𝙢𝙞𝙣𝙞𝙨𝘁𝗿𝗮𝗱𝗼𝗿 ⏤͟͟͞͞★"
    }
