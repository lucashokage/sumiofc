...
// Default fonts to use if Google Fonts aren't available
const DEFAULT_FONTS = {
...
// Create google-fonts-complete directory if it doesn't exist
const googleFontsDir = join(nodeModulesDir, "google-fonts-complete")
if (!existsSync(googleFontsDir)) {
  mkdirSync(googleFontsDir, { recursive: true })
}
...
const loadFonts = async () => {
  try {
    // Try to fetch fonts directly from Google Fonts CDN
    const buffers = await Promise.all([
      fetch(DEFAULT_FONTS["Roboto"]).then((res) => res.buffer()),
      fetch(DEFAULT_FONTS["Noto Sans"]).then((res) => res.buffer()),
    ])
...
const handler = async (msg, { conn, args }) => {
  ...
  const content = colores[firstWord]
  ...
  const titleFont = fontsLoaded ? 'bold 40px "Roboto", "Noto Sans", sans-serif' : "bold 40px system-ui"

  const contentFont = fontsLoaded ? 'bold 60px "Roboto", "Noto Sans", sans-serif' : "bold 60px system-ui"
  ...
  await conn.sendMessage(
    chatId,
    {
      ...
      caption: "🖼 Imagen generada con fuentes web",
    },
    { quoted: msg },
  )
}
