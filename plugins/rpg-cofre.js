const handler = async (m, { isPrems, conn }) => {
  if (!global.db.data.users[m.sender]) {
    throw `${global.emoji4} Usuario no encontrado.`;
  }
  ...
  if (Date.now() < timeToNextCofre) {
    ...
    const mensajeEspera = `《✧》 Ya reclamaste tu cofre\nⴵ Regresa en: *${msToTime(tiempoRestante)}* para volver a reclamar.`;
...
╰━━━━━━━━━━━━⬣`;
  ...
  try {
    await conn.sendFile(m.chat, img, 'yuki.jpg', texto, global.fkontak);
  } catch (error) {
    throw `${global.msm} Ocurrió un error al enviar el cofre.`;
  }
};
